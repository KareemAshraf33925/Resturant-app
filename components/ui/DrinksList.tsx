...
export default function DrinksList({ allProducts }: { allProducts: any[] }) {
  ...
  return (
    <div className='my-20  mx-8'>
      <h2 className="text-2xl font-bold mb-4 "> Drinks</h2>
        <p className='font-medium  text-orange-700'>It is known for its delicious flavor and ability to easily absorb spices,<br/> making it suitable for various cultures and tastes.</p>
        <p className='font-medium text-orange-700 mb-5'>It is also a high-protein, low-fat option, which makes it ideal for healthy diets.</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 ">
        {currentProducts.map((product,index) => (
          <div key={index} className="group relative shadow-md  rounded-xl">
              <img
                alt={product.name}
                src={product.img}
